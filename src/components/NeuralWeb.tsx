...
export const NeuralWeb: React.FC<NeuralWebProps> = ({ 
  ...
  const draw = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const currentIntensity = moodIntensityRef.current;
    ...
    const moodMultiplier = currentMood !== 'NORMAL' ? (1 + currentIntensity * 0.5) : 1;
    ...
    const opacity = currentIsSpeaking ? 0.8 + currentVolume * 0.2 : (currentMood !== 'NORMAL' ? Math.min(0.4 + currentIntensity * 0.2, 0.9) : 0.4);
